...
- **恢复了配置导入 / 导出。** 在 `/config` 页面将全部设置（不含翻译缓存）备份为带版本号的 JSON 文件，或从文件恢复。上游主动移除了配置备份功能，本 fork 以纯本地、最小化的方式重新加回，以便在多设备间迁移。
- **打包与提交准备。** 为避免名称冲突重命名、添加当前 AMO 政策所需的 Firefox manifest 字段、构建与隐私文档。
